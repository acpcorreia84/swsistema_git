...
/**
 * Base class that represents a row from the 'acao' table.
 *
 * 
 *
 * @package    pacoteComum.om
 */
abstract class BaseAcao extends BaseObject  implements Persistent {
	 ...
	 * @return     Acao The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = AcaoPeer::ID;
		}
	...
	public function setSigla($v)
	{
		...
		if ($this->sigla !== $v) {
			$this->sigla = $v;
			$this->modifiedColumns[] = AcaoPeer::SIGLA;
		}
	...
	public function setDescricao($v)
	{
		...
		if ($this->descricao !== $v) {
			$this->descricao = $v;
			$this->modifiedColumns[] = AcaoPeer::DESCRICAO;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = AcaoPeer::NUM_COLUMNS - AcaoPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Acao object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = AcaoPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				AcaoPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedRows += AcaoPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = AcaoPeer::doValidate($this, $columns)) !== true) {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AcaoPeer::ID)) $criteria->add(AcaoPeer::ID, $this->id);
		if ($this->isColumnModified(AcaoPeer::SIGLA)) $criteria->add(AcaoPeer::SIGLA, $this->sigla);
		if ($this->isColumnModified(AcaoPeer::DESCRICAO)) $criteria->add(AcaoPeer::DESCRICAO, $this->descricao);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(AcaoPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Acao (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Acao Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * Otherwise if this Acao has previously been saved, it will retrieve
	 * related LogSeramas from storage. If this Acao is new, it will return
		...
		if ($this->collLogSeramas === null) {
			if ($this->isNew()) {
			   $this->collLogSeramas = array();
				...
				$criteria->add(LogSeramaPeer::ACAO_ID, $this->id);
	 ...
	 * Otherwise if this Acao is new, it will return
	 * an empty collection; or if this Acao has previously
	 * been saved, it will retrieve related LogSeramas from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Acao.
	 */
	public function getLogSeramasJoinUsuario($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(AcaoPeer::DATABASE_NAME);
		}
		...
		if ($this->collLogSeramas === null) {
			if ($this->isNew()) {
				...
				$this->collLogSeramas = LogSeramaPeer::doSelectJoinUsuario($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
