...
abstract class BaseLogAcesso extends BaseObject  implements Persistent {
	...
	public function setAcao($v)
	{
		...
		if ($this->acao !== $v) {
			$this->acao = $v;
			$this->modifiedColumns[] = LogAcessoPeer::ACAO;
		}
	...
	public function setDescricao($v)
	{
		...
		if ($this->descricao !== $v) {
			$this->descricao = $v;
			$this->modifiedColumns[] = LogAcessoPeer::DESCRICAO;
		}
	...
	public function setTela($v)
	{
		...
		if ($this->tela !== $v) {
			$this->tela = $v;
			$this->modifiedColumns[] = LogAcessoPeer::TELA;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->acao = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			...
			return $startcol + 7; // 7 = LogAcessoPeer::NUM_COLUMNS - LogAcessoPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				LogAcessoPeer::doDelete($this, $con);
				$this->postDelete($con);
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = LogAcessoPeer::doInsert($this, $con);
					...
					$affectedRows += LogAcessoPeer::doUpdate($this, $con);
				}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(LogAcessoPeer::ACAO)) $criteria->add(LogAcessoPeer::ACAO, $this->acao);
		if ($this->isColumnModified(LogAcessoPeer::IP)) $criteria->add(LogAcessoPeer::IP, $this->ip);
		if ($this->isColumnModified(LogAcessoPeer::DESCRICAO)) $criteria->add(LogAcessoPeer::DESCRICAO, $this->descricao);
		if ($this->isColumnModified(LogAcessoPeer::DATA)) $criteria->add(LogAcessoPeer::DATA, $this->data);
		if ($this->isColumnModified(LogAcessoPeer::TELA)) $criteria->add(LogAcessoPeer::TELA, $this->tela);
