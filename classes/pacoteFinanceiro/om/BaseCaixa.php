 ...
 */
abstract class BaseCaixa extends BaseObject  implements Persistent {
	 ...
	 * @return     Caixa The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = CaixaPeer::ID;
		}
	...
	public function setUsuarioId($v)
	{
		...
		if ($this->usuario_id !== $v) {
			$this->usuario_id = $v;
			$this->modifiedColumns[] = CaixaPeer::USUARIO_ID;
		}
	...
	public function setDataAbertura($v)
	{
		...
		if ( $this->data_abertura !== null || $dt !== null ) {
			...
			$currNorm = ($this->data_abertura !== null && $tmpDt = new DateTime($this->data_abertura)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->data_abertura = ($dt ? $dt->format('Y-m-d H:i:s') : null);
				$this->modifiedColumns[] = CaixaPeer::DATA_ABERTURA;
			}
	...
	public function setDataFechamento($v)
	{
		...
		if ( $this->data_fechamento !== null || $dt !== null ) {
			...
			$currNorm = ($this->data_fechamento !== null && $tmpDt = new DateTime($this->data_fechamento)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->data_fechamento = ($dt ? $dt->format('Y-m-d H:i:s') : null);
				$this->modifiedColumns[] = CaixaPeer::DATA_FECHAMENTO;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->data_abertura = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->data_fechamento = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->resetModified();
			...
			return $startcol + 4; // 4 = CaixaPeer::NUM_COLUMNS - CaixaPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Caixa object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = CaixaPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				CaixaPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = CaixaPeer::doInsert($this, $con);
					...
					$affectedRows += CaixaPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = CaixaPeer::doValidate($this, $columns)) !== true) {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CaixaPeer::ID)) $criteria->add(CaixaPeer::ID, $this->id);
		if ($this->isColumnModified(CaixaPeer::USUARIO_ID)) $criteria->add(CaixaPeer::USUARIO_ID, $this->usuario_id);
		if ($this->isColumnModified(CaixaPeer::DATA_ABERTURA)) $criteria->add(CaixaPeer::DATA_ABERTURA, $this->data_abertura);
		if ($this->isColumnModified(CaixaPeer::DATA_FECHAMENTO)) $criteria->add(CaixaPeer::DATA_FECHAMENTO, $this->data_fechamento);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(CaixaPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Caixa (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setDataAbertura($this->data_abertura);

		$copyObj->setDataFechamento($this->data_fechamento);
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Caixa Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * Otherwise if this Caixa has previously been saved, it will retrieve
	 * related LancamentoCaixas from storage. If this Caixa is new, it will return
		...
		if ($this->collLancamentoCaixas === null) {
			if ($this->isNew()) {
			   $this->collLancamentoCaixas = array();
				...
				$criteria->add(LancamentoCaixaPeer::CAIXA_ID, $this->id);
