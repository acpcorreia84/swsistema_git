    ...
    if ($contaReceberObj->getCertificado()) {
        ...
        foreach ($certificadosSituacoesObj as $certificadoSituacao) {
            if ($certificadoSituacao->getUsuario())
                ...
                $usuario = '---';
                $situacoes[] = array('Id'=>$certificadoSituacao->getId(), 'Data'=>$certificadoSituacao->getData('d/m/Y H:i:s'), utf8_encode('Usu�rio')=>utf8_encode($usuario),
                    utf8_encode('Descri��o')=>utf8_encode($certificadoSituacao->getSituacao()->getNome()),
                    utf8_encode('Coment�rio')=>utf8_encode($certificadoSituacao->getComentario())
            );
        }

        $colunasSituacoes = array(
            array('nome'=>'Id'),array('nome'=>'Data'), array('nome'=>utf8_encode('Usu�rio')), array('nome'=>utf8_encode('Descri��o')),
            array('nome'=>utf8_encode('Coment�rio'))
        );
        ...
        if ($certificado->getFormaPagamentoId() == 1) {
            ...
            foreach ($boletosObj as $boleto) {
                ...
                $boletos[] = array(
                    ...
                    'valor'=>formataMoeda($boleto->getValor()),utf8_encode('Situa��o')=>$situacaoBoleto,
                ...
                $colunasBoletos = array(
                    ...
                    array('nome'=>utf8_encode('Situa��o')), array('nome'=>'D. Pagt')
                );
            ...
            if ( $contaReceber->getCertificado() ) {
                ...
                if ($adicionarConta)
                    $contasReceber[] =  array(' '=>(++$key),'Id'=>$contaReceber->getId(),'Sit.'=>$situacaoPagamento, 'Data'=> ($contaReceber->getDataDocumento('d/m/Y'))?$contaReceber->getDataDocumento('d/m/Y'):'---',
                        utf8_encode('Descri��o')=> utf8_encode($descricao), 'Valor'=> formataMoeda($certificado->getProduto()->getPreco()), 'Venc.'=>$contaReceber->getVencimento('d/m/Y'),
                        ...
                        utf8_encode('A��o')=>'<button onclick="carregarDetalhesContaReceber(\''.$contaReceber->getId().'\'); $(\'#modalContaReceberDetalhar\').modal(\'show\')"><i class="fa fa-arrows"></i></button> '
                );
            }
        }

        $colunas = array(
            array('nome'=>' '),array('nome'=>'Id'), array('nome'=>'Sit.'),array('nome'=>'Comp.'), array('nome'=>'Data'), array('nome'=>utf8_encode('Descri��o')),
...
function extornarContaReceber() {
    try {
        ...
        $lancamentoConta = new LancamentoConta();
        $lancamentoConta->setDataLancamento($dataLancamento);
        $lancamentoConta->setDescricao(utf8_decode('Extorno de lan�amento da conta: ' . $contaReceber->getDescricao() ) );
